...
class StickyCookie(BaseHTTPMiddleware):
    def __init__(self, app:App, value:str, key:str='sticky', verbose:bool=False):
        super().__init__(app)
        self.value = value
        self.key = key
    ...
    async def dispatch(self, request, call_next):
        ...
        if sticky and self.verbose:
            ...
            if sticky != self.value:
                ...
                click.secho(f'sticky missmatch: {sticky} != {self.value}', fg='red', bold=True)
            else:
                click.secho(f'sticky match: {sticky} == {self.value}', fg='green', bold=True)
...
def init_sticky(app:App) -> App:
    ...
    app.starlette_app.user_middleware.append(
        Middleware(StickyCookie, value=INSTANCE_COOKIE, key="sticky", verbose=True)
    )
    return app
